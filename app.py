...
st.markdown('<div class="main-title">AI HR Resume Shortlisting Agent</div>', unsafe_allow_html=True)
st.markdown(
...
if jd_input:
    try:
        ...
        st.markdown('<div class="section-header">Parsed Job Requirements</div>', unsafe_allow_html=True)
        st.json(jd_data)
...
if uploaded_files and jd_data:
    ...
    st.markdown('<div class="section-header">Detailed Candidate Evaluation</div>', unsafe_allow_html=True)

    for item in detailed_results:
        candidate = item["candidate"]
        score = item["score"]

        with st.expander(f"{candidate['name']} | Score: {score['total_score']}"):
